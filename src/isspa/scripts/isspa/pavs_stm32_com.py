...
class PAVS(object):
    ...
    def __init__(self, car_type=1, com="/dev/myserial", delay=.002, debug=False):
        ...
        if self.__debug:
            logger.debug("cmd_delay = " + str(self.__delay_time) + " s")

        if self.ser.isOpen():
            logger.info("PAVS STM32 Serial Opened! Baudrate=115200")
        else:
            logger.fatal("Serial Open Failed!")
...
if __name__ == '__main__':
    ...
    # Car's low-level processing library
    max_com_index = 256
    com_index = 1
    
    while com_index <= max_com_index:
        try:
            logger.info(f"Trying COM{com_index}")
            com_port = f'COM{com_index}'
            bot_instance = PAVS(1, com_port, debug=True)
            break
        except Exception as e:
            if com_index == max_com_index:
                logger.warning("-----------------------No COM Port Open--------------------------")
                exit(0)
            com_index += 1
            continue
    
    logger.info(f"--------------------Opened {com_port}---------------------")


    bot_instance.create_receive_threading()
    time.sleep(.1)
    bot_instance.set_beep(50)
    time.sleep(.1)

    version = bot_instance.get_version()
    logger.info("version=", version)

    try:
        while True:
            ax, ay, az = bot_instance.get_accelerometer_data()
            gx, gy, gz = bot_instance.get_gyroscope_data()
            mx, my, mz = bot_instance.get_magnetometer_data()

            logger.info("%3.3f, %3.3f, %3.3f, %3.3f, %3.3f, %3.3f, %3.3f, %3.3f, %3.3f" % 
            (ax, ay, az, gx, gy, gz, mx, my, mz))

            time.sleep(.1)

    except KeyboardInterrupt:
        bot_instance.set_car_motion(0, 0, 0)
        pass
    exit(0)
