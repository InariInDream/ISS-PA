...
from vehicle_msgs.msg import CenterLine, CenterLinePoint
# , Obstacles, DynamicObstacles, DynamicObstacle, \
#     DynamicTrajectoryPoint
from geometry_msgs.msg import Polygon, Point32


def generate_center_line(config, start_x=0.0, start_y=0.0, start_yaw=0.0, resolution=0.1, left_bound=0.75,
                         right_bound=0.75):
    x = start_x
    y = start_y
    yaw = start_yaw
    ...
    for seg in config:
        if isinstance(seg, list):
            ...
            angles = np.linspace(start_angle, end_angle, int(point_count))
...
def main():
    ...
    if 'path' in sys.argv:
        ...
        left_ref_pub = rospy.Publisher('/left_line_path', Path, queue_size=1, latch=True)
        left_msg = Path()
        ...
        for pt in center_line.points:
            ...
            pose.pose.position.x = pt.x - pt.left_bound * np.sin(pt.theta)
            pose.pose.position.y = pt.y + pt.left_bound * np.cos(pt.theta)
            left_msg.poses.append(pose)
        left_ref_pub.publish(left_msg)
        
        right_ref_pub = rospy.Publisher('/right_line_path', Path, queue_size=1, latch=True)
        right_msg = Path()
        right_msg.header.stamp = rospy.Time.now()
        right_msg.header.frame_id = 'odom'
        for pt in center_line.points:
            pose = PoseStamped()
        ...
        right_ref_pub.publish(right_msg)
